...
def display_conversation(log_entry: Dict[str, Any]):
    ...
    if log_entry.get('event_type') == 'conversation':
        ...
        print(f"   Stream: {log_entry.get('stream', False)} | Endpoint: {log_entry.get('endpoint', 'unknown')}")
        print(f"👤 User: {log_entry.get('user_message', '')}")
        print(f"🤖 Assistant: {log_entry.get('assistant_response', '')[:200]}{'...' if len(log_entry.get('assistant_response', '')) > 200 else ''}")
        print(f"   Lengths: User={log_entry.get('user_message_length', 0)} | Assistant={log_entry.get('response_length', 0)}")
        ...
        print(f"   Input: {str(log_entry.get('tool_input', ''))[:100]}{'...' if len(str(log_entry.get('tool_input', ''))) > 100 else ''}")
        print(f"   Output: {log_entry.get('tool_output', '')[:100]}{'...' if len(log_entry.get('tool_output', '')) > 100 else ''}")
